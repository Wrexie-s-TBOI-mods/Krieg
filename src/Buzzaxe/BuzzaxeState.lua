...
local Util = include "MeleeWeaponApi.Util"
...
function Buzzaxe:shouldReleaseTheBeast(player)
    ...
    return hearts <= Constants.RTB_THRESHOLD
end
...
function Buzzaxe:GetSlot(player)
    for _, slot in pairs(ActiveSlot) do
        if player:GetActiveItem(slot) == self.ID then return slot end
    end
end
...
function Buzzaxe:isRampaging(player)
    return player:GetEffects():HasCollectibleEffect(self.ID)
end
...
function Buzzaxe:CleanupState(player)
    ...
    state.beast = false
    state.shoot = false
    state.clock:reset(Constants.CLOCK_ACTIVE_CHARGE_TICKS)
